...
})({"dLN92":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "b82fa643f0746dda";
"use strict";
...
},{}],"4H4cL":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "HomeController", ()=>HomeController);
...
const HomeController = {
    ...
    bindEventListeners () {
        ...
        if (subContainer) subContainer.addEventListener("click", handleSubClick.bind(this));
        ...
        if (postContainer) postContainer.addEventListener("click", handlePostClick.bind(this));
    },
    async loadInitialData () {
        // Load video data and wait for it to complete
        await this.loadCurrentPageData();
        // Once video data is loaded, load other data in parallel
        const subAndStoryData = await Promise.all(await (0, _model.YoutubeModel).getSubAndStoryData());
        // Load subscription data
        const subscriptionContent = (0, _view.YoutubeView).renderSubs(subAndStoryData);
        document.querySelector(".subscriptions").insertAdjacentHTML("beforeend", subscriptionContent);
        // Load story data
        const storyContent = (0, _view.YoutubeView).renderStories(subAndStoryData);
        document.querySelector(".story-gallery").insertAdjacentHTML("beforeend", storyContent);
        // Initialize other UI elements
        document.querySelector(".pagination").innerHTML = (0, _view.YoutubeView).renderPagination();
        this.initPagination();
    ...
    initializeSubButtons () {
        ...
        subBtns.forEach((subBtn)=>{
            const channelId = subBtn.getAttribute("channel-id");
            const isSubscribed = (0, _model.YoutubeModel).getSubStatus(channelId) === "subscribed";
            (0, _view.YoutubeView).updateUIForSubButton(subBtn, isSubscribed);
        });
    },
    initializeLikeButtons () {
        ...
        likeBtns.forEach((likeBtn)=>{
            const videoId = likeBtn.querySelector("[data-video-id]").dataset.videoId;
            const isLiked = (0, _model.YoutubeModel).getLikeStatus(videoId) === "like";
            (0, _view.YoutubeView).updateUIForLikeButton(likeBtn.querySelector(".fa-thumbs-up"), isLiked);
        });
    },
    handleDarkModeToggle () {
        const isDarkMode = document.body.classList.contains("dark-theme");
        (0, _view.YoutubeView).updateUIForDarkMode(!isDarkMode);
    ...
    toggleSub (subBtn, channelId) {
        const newStatus = (0, _model.YoutubeModel).toggleSubStatus(channelId);
        const isSubscribed = newStatus === "subscribed";
        (0, _view.YoutubeView).updateUIForSubButton(subBtn, isSubscribed);
    },
    toggleLike (likeBtn, videoId) {
        const newStatus = (0, _model.YoutubeModel).toggleLikeStatus(videoId);
        const isLike = newStatus === "like";
        (0, _view.YoutubeView).updateUIForLikeButton(likeBtn, isLike);
    },
    initPagination () {
        this.setupPaginationClickHandler(document.querySelector(".pagination"));
    },
    setupPaginationClickHandler (pagContainer) {
        (0, _view.YoutubeView).addHandlerClick(async (pageNumber)=>{
            (0, _model.YoutubeModel).setCurrentPage(pageNumber);
            await this.loadCurrentPageData();
            pagContainer.innerHTML = (0, _view.YoutubeView).renderPagination();
        });
    },
    async loadCurrentPageData () {
        try {
            const videoPageData = await (0, _model.YoutubeModel).getVideoData();
    ...
    async renderPageData (pageData) {
        ...
        for (const videoData of pageData.items)try {
            const channelData = await (0, _model.YoutubeModel).getChannelData(videoData.snippet.channelId);
            const videoContent = (0, _view.YoutubeView).renderVideo(videoData, channelData);
...
},{"regenerator-runtime/runtime":"dXNgZ","./view":"ky8MP","./model":"Y4A21","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dXNgZ":[function(require,module,exports) {
/**
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "YoutubeView", ()=>YoutubeView);
var _runtime = require("regenerator-runtime/runtime");
var _iconsSvg = require("../assets/imgs/icons.svg");
...
const YoutubeView = {
    renderVideo (videoData, headerData, i) {
        ...
        const formattedTags = (tags || []).slice(0, 2).map((tag)=>`<a href="#">#${tag}</a>`).join(" ");
        return `
        <div class="post-container">
            ...
            <div class="post-row">
                <div class="activity-icons">
                    <div><i class="fa-solid fa-eye"></i> ${this.formatCount(viewCount)}</div>
                    <div><button class="like-btn"><i class="fa-solid fa-thumbs-up" data-video-id="${videoId}"></i></button> ${this.formatCount(likeCount)}</div>
                    <div><i class="fa-solid fa-comments"></i> ${this.formatCount(commentCount)}</div>
                </div>
    ...
    calculateTimeAgo (publishedAt) {
        ...
        else if (weeks < 4) return weeks === 1 ? "1 week ago" : `${weeks} weeks ago`;
        else if (months < 12) return months === 1 ? "1 month ago" : `${months} months ago`;
        else return years === 1 ? "1 year ago" : `${years} years ago`;
    },
    formatCount (count) {
        if (count < 1000) return count; // return the original number if less than 1000
        else if (count < 1000000) return (count / 1000).toFixed(1) + "K"; // convert to K for thousands
        else return (count / 1000000).toFixed(1) + "M"; // convert to M for millions
    },
    renderSubs (subData) {
    ...
    renderStories (storyData) {
        return storyData.map((data, index)=>{
            if (index < 4) return `
            <div class="story story${index + 1}" style="background-image: linear-gradient(transparent, rgba(0, 0, 0, 0.5)), url(${data.profilePic})">
    ...
    addHandlerClick (handler) {
        this.paginationContainer.addEventListener("click", async function(e) {
            ...
            if (!btn) return;
            const data = await (0, _model.YoutubeModel).fetchYouTubeData();
            if (btn.classList.contains("pagination__btn--next")) await (0, _model.YoutubeModel).setPageToken(data.nextPageToken);
            else if (btn.classList.contains("pagination__btn--prev")) await (0, _model.YoutubeModel).setPageToken(data.prevPageToken);
            const goToPage = +btn.dataset.goto;
            handler(goToPage);
        });
    },
    renderPagination () {
        const currPage = (0, _model.YoutubeModel).getCurrentPage();
        const numPages = Math.ceil((0, _model.YoutubeModel).getTotalResults() / (0, _model.YoutubeModel).getResultsPerPage());
        const prevBtn = `
          ...
          `;
        const fwrdBtn = `<button data-goto="${currPage + 1}" class="btn--inline pagination__btn--next">
                  <span>Page ${currPage + 1}</span>
                  <svg class="search__icon">
        ...
        if (currPage === 1 && numPages > 1) return fwrdBtn;
        //Last page
        if (currPage === numPages && numPages > 1) return prevBtn;
        //Other page
        if (currPage < numPages) return prevBtn + fwrdBtn;
        // Page 1, and there are no other pages
        return "";
    }
};

},{"regenerator-runtime/runtime":"dXNgZ","../assets/imgs/icons.svg":"hlE6o","./model":"Y4A21","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hlE6o":[function(require,module,exports) {
module.exports = require("8f33fc778e4b29a4").getBundleURL("fOpWw") + "icons.8abafeca.svg" + "?" + Date.now();

},{"8f33fc778e4b29a4":"lgJ39"}],"lgJ39":[function(require,module,exports) {
"use strict";
var bundleURL = {};
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "YoutubeModel", ()=>YoutubeModel);
var _runtime = require("regenerator-runtime/runtime");
"use strict";
// Model
...
const YoutubeModel = {
    ...
    async fetchYouTubeData () {
        try {
            ...
            if (!response.ok) throw new Error(`Error fetching YouTube data: ${data.error.message}`);
            return data;
        } catch (error) {
            console.error(error);
            return null;
    ...
    async getChannelData (channelId) {
        try {
            ...
            else throw new Error(`Error fetching channel data: ${data.error.message}`);
        } catch (error) {
            console.error(error);
            return null;
        }
    ...
    async getSubAndStoryData () {
        try {
            ...
            return data.items.map(async (item)=>await this.getChannelData(item.snippet.channelId));
        } catch (error) {
            console.error(error);
            return null;
        }
...
},{}]},["dLN92","4H4cL"], "4H4cL", "parcelRequireafe4")

//# sourceMappingURL=index.f0746dda.js.map
