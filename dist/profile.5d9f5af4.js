...
})({"2VtNu":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "950cf21a5d9f5af4";
"use strict";
...
},{}],"omNiE":[function(require,module,exports) {
...
parcelHelpers.export(exports, "getVideoData", ()=>getVideoData);
...
const getChannelData = async function(channelId) {
    try {
        const res = await fetch(`https://youtube.googleapis.com/youtube/v3/channels?part=snippet%2CcontentDetails%2Cstatistics&id=${channelId}&key=${API_KEY}`);
...
const renderVideoContent = function(videoData, headerData, index) {
    ...
    if (videoSnippet != undefined) {
        ...
        if (days == 0 && hours == 0 && minutes == 0) {
            time = seconds;
            tag = " seconds ago";
            if (seconds == 1) tag = " second ago";
        } else if (days == 0 && hours == 0) {
            time = minutes;
            tag = " minutes ago";
            if (minutes == 1) tag = " minute ago";
        } else if (days == 0) {
            time = hours;
            if (hours == 1) tag = " hour ago";
            tag = " hours ago";
        } else {
            time = days;
            tag = " days ago";
            if (days == 1) tag = " day ago";
        }
        const countTrunc = function(count) {
            if (count >= 1000000) {
                count = (count / 1000000).toFixed(1);
                if (count.charAt(count.length - 1) == "0") count = Math.trunc(count);
        ...
        if (videoSnippet?.tags != undefined) {
            if (videoSnippet?.tags[0] != undefined) tag1 = "#" + videoSnippet?.tags[0];
            if (videoSnippet?.tags[1] != undefined) tag2 = "#" + videoSnippet?.tags[1];
        }
        const markup = `
    <div class="post-container">
      ...
      <div class="post-row">
        <div class="activity-icons">
          ...
          <div><button class="like-btn"><i class="fa-solid fa-thumbs-up" data-video-id="video${index}"></i></button> ${countTrunc(likeCount)}</div>
          <div><i class="fa-solid fa-comments"></i> ${countTrunc(commentCount)}</div>
        </div>
...
const getVideoData = async function(container, index) {
    try {
...
const initializeLikeButtons = function() {
    const applyLikeButtonBehavior = function(postElement) {
        ...
        if (localStorage.getItem(localStorageKey) === "on") likeBtn.classList.add("like-on");
        likeBtn.addEventListener("click", function() {
            likeBtn.classList.toggle("like-on");
            if (localStorage.getItem(localStorageKey) === "on") localStorage.setItem(localStorageKey, "off");
            else localStorage.setItem(localStorageKey, "on");
        });
    };
    const observePostsContainer = function() {
        ...
        const observer = new MutationObserver(function(mutationsList) {
            for (const mutation of mutationsList)if (mutation.type === "childList") mutation.addedNodes.forEach((addedNode)=>{
                if (addedNode instanceof HTMLElement) applyLikeButtonBehavior(addedNode);
            });
        });
...
for(let index = 0; index < 5; index++)getVideoData(postsContainer, index);
...
const renderStories = function(storyData, videoData, index) {
    ...
    if (backgroundImage == undefined) backgroundImage = videoData.items[index].snippet.thumbnails.high.url;
    const markup = `
  <div class="story story${index + 2}" style="background-image: linear-gradient(transparent, rgba(0, 0, 0, 0.5)), url(${backgroundImage})">
...
},{}]},["2VtNu","omNiE"], "omNiE", "parcelRequireafe4")

//# sourceMappingURL=profile.5d9f5af4.js.map
